{if $responsivetype!='select' && $responsiveselect}
   ...
   <div id="menu-mobile" class="{$responsivetype} {if $responsiveselectsize=='phone'}visible-xs{elseif $responsiveselectsize=='tablet'}hidden-md{/if}">
        ...
        <ul>
            {foreach $mainnav k n mainnav}<li id="itemNo{$k}" class="{if $.foreach.mainnav.last}last {elseif $.foreach.mainnav.first}first {/if}{if $n.pageid==$pageid}current {/if}{if $n.subnav}hassubnav{/if}" ><a href="{$n.url}" title="{$n.title}"{if $n.pg_followto=='no'} rel="nofollow"{/if}>{$n.label}<span></span></a>{if $n.subnav }<a href="#" class="subnavtoggle"></a>
                <ul{if in_array($n.pageid, $selectedpages)} class="active"{/if}>
                    {foreach $n.subnav sk s subnav}<li class="{if $s.pageid==$pageid}current {/if}{if $.foreach.subnav.last}last {/if}{if $s.subnav}hassubnav{/if}"><a href="{$s.url}" title="{$s.title}">{$s.label}<span></span></a>{if $s.subnav }<a href="#" class="subnavtoggle"></a>
                        <ul>
                            {foreach $s.subnav tk t subsubnav}<li class="{if $t.pageid==$pageid}current {/if}{if $.foreach.subsubnav.last}last {/if}{if $t.subnav}hassubnav{/if}"><a href="{$t.url}" title="{$t.title}">{$t.label}<span></span></a>{if $t.subnav }<a href="#" class="subnavtoggle"></a>
